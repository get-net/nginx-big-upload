...
local crc32, sha1_handler, sha256_handler
...
function _M.new(self, handlers)
  ...
  if not session_id then
    ...
    if not session_id:match("^[a-zA-Z0-9-]+$") then
      return nil, {
        412,
        string.format(
          "Session-id is invalid, only ASCII alphanumeric characters or hypen are accepted, was %s",
          session_id
        )
      }
    end
  ...
  if range_from == 0 and range_to == 0 and range_total == 0 then
    ...
    if content_length - 1 ~= range_to - range_from then
      return nil, {
        412,
        string.format(
          "Range size does not match Content-Length (%d-%d/%d vs %d)",
          range_to,
          range_from,
          range_total,
          content_length
  ...
  ctx.get_name = function()
    ...
    if content_disposition then
      ...
      local mname =
        string.match(content_disposition, '%s*%w+%s*;%s*%w+%s*=%s*"?([^"]+)') or
        string.match(content_disposition, "%s*%w+%s*;%s*%w+%*%s*=%s*UTF%-8''(.+)") -- eventual UTF8 case
      if mname then
        return mname
      end
  ...
  if last_checksum or checksum then
    crc32 = require("crc32")
  end
  ...
  if xsha1 then
    sha1_handler = require("sha1_handler")
  ...
  if xsha256 then
    sha256_handler = require("sha256_handler")
...
function _M.process(self)
  ...
  for i, h in ipairs(self.handlers) do
    ...
    if result then
      return result
    end -- something important happened to stop upload
  end
